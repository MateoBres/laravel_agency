...
//    );
    // QUERY BUILDER
      ...
      return redirect('adminRegions')
            ->with('message', 'Region: ' .$regName. ' added successfully.');
});
...
Route::post('/deleteRegion', function(){
    ...
    // EXISTS
     if(DB::table('destinations')->where('regID', $regID)->exists()){
          return redirect('/adminRegions')
                     ->with(
                        [
                            'message'=>'The region '.$regName.' cannot be removed because has a assigned destination.'
                        ]
                    );
    };
    ...
    return redirect('/adminRegions')
                ->with(
                    [
                        'message'=>'The region '.$regName.' was deleted successfully.'
                    ]
                );
});
...
Route::post('/addDestination', function(){
    ...
    return redirect('adminDestinations')->with(
                            [
                                'message'=>'Destination: ' .$destName. ' added successfully'
                            ]
    );
});
...
Route::post('/modifyDestination', function(){
    ...
    return redirect('adminDestinations')
        ->with('message', 'Destination: '.$destName.
            ' modified successfully.');
});
...
Route::post('/deleteDestination', function(){
    ...
    return redirect('/adminDestinations')
        ->with(
            [
                'message'=>'The destination '.$destName.' was deleted successfully.'
            ]
        );
});
